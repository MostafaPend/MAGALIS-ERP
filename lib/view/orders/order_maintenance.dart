...
class OrderShipment extends StatefulWidget {
  @override
  State<OrderShipment> createState() => _OrderMaintenanceState();
}

class _OrderMaintenanceState extends State<OrderShipment> {
  String? selectTalab;
  String? stateTalab;
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          ...
                          DefaultContainer(title: 'تحصيل الطلب'),
                          Padding(
                          ...
                          Botton(
                            ...
                            onTap: () async{
                              if(payment==null){
                                ...
                                await FirebaseFirestore.instance.collection("courriers").doc(pro.courriers).update(
                                    {
                                      "balance":FieldValue.increment(-(pro.netamount!))
                                    });

                                await FirebaseFirestore.instance.collection("treasuryactions").doc(payment!).update(
                                    {
                                      "balance":FieldValue.increment(pro.netamount!)
                                    });

                                var rr = await FirebaseFirestore.instance.collection("treasuryactions").doc(payment!).get();

                                await FirebaseFirestore.instance.collection("treasuryactions").doc(payment!).collection("Reports").add(
                                    {
                                      "date":DateTime.now().toString(),
                                      "beforeBalance":rr["balance"],
                                      "afterBalance":rr["balance"]+pro.netamount!,
                                      "actionType":"supplychain",
                                ...
                                await FirebaseFirestore.instance.collection("courrierbalanceaction").doc(pro.id).update(
                                    {
